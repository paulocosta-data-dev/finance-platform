...
- Packaging as standalone executable (currently needs Python + `flet run flet_app.py`)
- Rule review / conflict resolution UI
...
`app/schema/versions.py` defines `CURRENT_TRANSACTION_SCHEMA_VERSION = 1` but there is no migration logic. If the Transaction model changes (new field, renamed field), existing parquet files will silently break or cause Pydantic errors on load.

What to build:
- A `schema/migrations.py` that detects parquet schema version and upgrades it
- Run migration check at app startup before any page loads
- Show a migration progress indicator in the UI if needed
...
### 4. Learned rules lifecycle

`app/category/services/learned_rule_service.py` appends rules but there is no deduplication, no conflict resolution, and no UI to review or delete them. Over time this file will accumulate contradictory rules.

What to build:
- A "Learned Rules" page showing all rules with the ability to disable or delete
- Conflict detection: two rules matching the same description but different categories
- Confidence decay for old rules that contradict newer corrections
